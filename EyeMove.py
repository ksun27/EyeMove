...
while True:
    ...
    for (fax, fay, faw, fah) in faces:
        cv2.rectangle(frame, (fax + (faw / 2), fay), (fax + (faw / 2), fay + fah), (0, 255, 0), 2)
        cv2.rectangle(frame, (fax, fay - offset), (fax + faw, fay - offset), (0, 255, 0), 2)
        facecoord = fay
        if cali == 0:
            print("Calibrated.")
            ax = fax + (faw / 2)
    ...
    try:
        ...
        midx = (x1 + x2) / 2
        midy = (y1 + y2) / 2
        cv2.rectangle(frame, (midx - 1, midy - 1), (midx + 1, midy + 1), (0, 0, 255), 2)
        eyecoord = midy
        spx = 383 + 20 * (ax - midx)
        spy = -5 * (ay - midy)
